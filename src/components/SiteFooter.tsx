  ...
  return (
    <footer className="border-t border-border mt-16">
      <div className="mx-auto max-w-6xl px-4 py-8 text-sm text-muted-foreground flex flex-wrap justify-between gap-4">
        <p>© {new Date().getFullYear()} MatrixDojo — fast, free linear algebra in your browser.</p>
        <nav className="flex flex-wrap gap-4">
          <Link to="/" className="hover:text-foreground">Multiply</Link>
          <Link to="/add" className="hover:text-foreground">Add</Link>
          <Link to="/scalar" className="hover:text-foreground">Scalar</Link>
          <Link to="/power" className="hover:text-foreground">Power</Link>
          ...
          <Link to="/trace-rank" className="hover:text-foreground">Trace / Rank</Link>
        </nav>
      </div>
    </footer>
  );

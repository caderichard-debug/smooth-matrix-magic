...
import { determinant, dims, eAdd, eMul, formatExpr, fromNumbers, parseExpr, type Matrix } from "@/lib/matrix";
...
export const Route = createFileRoute("/determinant-expansion")({
  head: () => ({
    meta: [
      ...
      { property: "og:description", content: "See every cofactor term in a row/column determinant expansion." },
    ],
  }),
...
function DeterminantExpansionPage() {
  ...
  const out = useMemo(() => {
    try {
      ...
      return { terms: [], expandedDet: null, directDet: null, error: e instanceof Error ? e.message : "Error" };
    }
  }, [a, mode, indexText]);
    ...
    >
      <div className="grid md:grid-cols-3 gap-4">
        <div className="rounded-lg border border-border bg-card/40 p-6 space-y-2">
          <Label className="text-xs uppercase tracking-wider text-muted-foreground">Expansion mode</Label>
          <div className="flex gap-2">
            <Button type="button" variant={mode === "row" ? "default" : "secondary"} onClick={() => setMode("row")}>Row</Button>
            <Button type="button" variant={mode === "col" ? "default" : "secondary"} onClick={() => setMode("col")}>Column</Button>
          </div>
        </div>
        <div className="rounded-lg border border-border bg-card/40 p-6 space-y-2">
          <Label className="text-xs uppercase tracking-wider text-muted-foreground">{mode === "row" ? "Row index (1-based)" : "Column index (1-based)"}</Label>
          <Input value={indexText} onChange={(e) => setIndexText(e.target.value)} className="font-mono" />
        </div>
      </div>
      ...
      <section className="rounded-lg border border-border bg-card/40 p-6 space-y-4">
        ...
        {out.error ? (
          ...
          <>
            <p className="text-sm text-muted-foreground font-mono">Expanded determinant = {out.expandedDet ? formatExpr(out.expandedDet) : ""}</p>
            <p className="text-sm text-muted-foreground font-mono">Direct determinant = {out.directDet ? formatExpr(out.directDet) : ""}</p>
            <ul className="list-disc pl-5 space-y-1 text-sm text-muted-foreground">
              {out.terms.map((term) => <li key={term} className="font-mono">{term}</li>)}
            </ul>
          </>
        )}
      </section>
      ...
      <section className="rounded-lg border border-border bg-card/40 p-6 space-y-3">
        ...
        <p className="text-sm text-muted-foreground">
          Laplace expansion writes <span className="font-mono">det(A)</span> as a sum of entries times cofactors along any chosen row or column.
          Cofactors include the alternating sign pattern <span className="font-mono">(-1)^(i+j)</span>.
        </p>
        <p className="text-sm text-muted-foreground">
          Picking a row or column with many zeros reduces work because zero entries contribute zero terms. This identity is
          exact and works symbolically, not only numerically.
        </p>
      </section>

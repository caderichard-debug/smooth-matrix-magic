...
export const Route = createFileRoute("/block-matrix-builder")({
  head: () => ({
    meta: [
      ...
      { property: "og:description", content: "Assemble [[A,B],[C,D]] with row/column compatibility checks." },
    ],
  }),
...
function BlockMatrixBuilderPage() {
  ...
  const out = useMemo(() => {
    try {
      ...
      return { block: null, top: null, bottom: null, error: e instanceof Error ? e.message : "Error" };
    }
  }, [a, b, c, d]);
    ...
    >
      ...
      <section className="rounded-lg border border-border bg-card/40 p-6 space-y-4">
        ...
        {out.error ? (
          ...
          <div className="grid lg:grid-cols-2 gap-4">
            {out.top && <div className="overflow-x-auto"><MatrixDisplay m={out.top} label="[A | B]" /></div>}
            {out.bottom && <div className="overflow-x-auto"><MatrixDisplay m={out.bottom} label="[C | D]" /></div>}
            {out.block && <div className="overflow-x-auto"><MatrixDisplay m={out.block} label="[[A, B], [C, D]]" /></div>}
          </div>
        )}
      </section>
      ...
      <section className="rounded-lg border border-border bg-card/40 p-6 space-y-3">
        ...
        <p className="text-sm text-muted-foreground">
          A 2x2 block matrix is assembled by first building the top row <span className="font-mono">[A|B]</span> and bottom row
          <span className="font-mono"> [C|D]</span>, then stacking them vertically.
        </p>
        <p className="text-sm text-muted-foreground">
          This requires row compatibility within each block row and column compatibility across block rows. Block matrices
          are the natural language of Schur complements and partitioned linear solves.
        </p>
      </section>

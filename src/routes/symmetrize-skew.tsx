...
import { add, dims, fromNumbers, parseExpr, scalarDivide, subtract, transpose, type Matrix } from "@/lib/matrix";
...
export const Route = createFileRoute("/symmetrize-skew")({
  head: () => ({
    meta: [
      ...
      { property: "og:description", content: "Compute A = S + K with S symmetric and K skew-symmetric." },
    ],
  }),
...
function SymmetrizeSkewPage() {
  const [a, setA] = useState<Matrix>(() => fromNumbers([[2, 3, 1], [0, -1, 4], [5, 2, 7]]));
  ...
  const out = useMemo(() => {
    try {
      ...
      return { symmetric: null, skew: null, reconstructed: null, error: e instanceof Error ? e.message : "Error" };
    }
  }, [a]);
    ...
    >
      ...
      <section className="rounded-lg border border-border bg-card/40 p-6 space-y-4">
        ...
        {out.error ? (
          ...
          <div className="grid lg:grid-cols-2 gap-4">
            {out.symmetric && <div className="overflow-x-auto"><MatrixDisplay m={out.symmetric} label="S = (A + A^T)/2" /></div>}
            {out.skew && <div className="overflow-x-auto"><MatrixDisplay m={out.skew} label="K = (A - A^T)/2" /></div>}
            {out.reconstructed && <div className="overflow-x-auto"><MatrixDisplay m={out.reconstructed} label="S + K (reconstruction)" /></div>}
          </div>
        )}
      </section>
      ...
      <section className="rounded-lg border border-border bg-card/40 p-6 space-y-3">
        ...
        <p className="text-sm text-muted-foreground">
          Every square matrix can be uniquely decomposed as <span className="font-mono">A = S + K</span> where
          <span className="font-mono"> S^T = S</span> and <span className="font-mono">K^T = -K</span>.
        </p>
        <p className="text-sm text-muted-foreground">
          The formulas are <span className="font-mono">S = (A + A^T)/2</span> and <span className="font-mono">K = (A - A^T)/2</span>.
          This decomposition is central in continuum mechanics, quadratic forms, and Lie algebra structure.
        </p>
      </section>
